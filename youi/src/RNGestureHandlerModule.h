...
public:
    ...
    YI_RN_EXPORT_METHOD(createGestureHandler)(std::string name, uint64_t tag, const folly::dynamic &config);
    
    YI_RN_EXPORT_METHOD(attachGestureHandler)(uint64_t handlerTag, uint64_t viewTag);
    
    YI_RN_EXPORT_METHOD(updateGestureHandler)(uint64_t handlerTag, const folly::dynamic &newConfig);
